  ...
  _createClass(MovieAPI, [{
    ...
    value: function _removeShowThumbnail(showID) {
      return this._showsModel.getSingle(showID).then(function (show) {
        var fullPath = _path["default"].join(_thumbnails["default"].shows, show.image_filename);

        if (!_fs["default"].existsSync(fullPath)) {
          return Promise.resolve(true);
        }

        return Promise.resolve(_fs["default"].unlinkSync(fullPath));
      });
    }
  }, {
    ...
    value: function _removeMovieThumbnail(movie) {
      var fullPath = _path["default"].join(_thumbnails["default"].movies, movie.image_filename);
