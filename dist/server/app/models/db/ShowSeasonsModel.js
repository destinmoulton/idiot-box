...
export default class ShowSeasonsModel {
    ...
    async addShowSeason(showID, apiData) {
        ...
        const season = await this.getSingleByShowSeasonTrakt(showID, apiData.number, apiData.ids.trakt);
        if ("id" in season) {
            return season;
        }
        await this._ibdb.insert(data, this._tableName);
        return await this.getSingleByShowSeasonTrakt(showID, data.season_number, data.trakt_id);
    }
    ...
    async getSeasonsForShow(showID) {
        ...
        return await this._ibdb.getAll(where, this._tableName, "season_number ASC");
    }
