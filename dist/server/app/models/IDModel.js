...
class IDModel {
    ...
    async idAndArchiveMovie(movieInfo, imageURL, sourceInfo, destInfo) {
        ...
        await this._filesystemModel.moveInSetDir(sourceInfo, destInfo, "Movies");
        let newFilename = "";
        if (imageURL !== "") {
            newFilename = await this._mediaScraperModel.downloadThumbnail("movies", imageURL, imageFilename);
        }
        const movieRow = await this._moviesModel.addMovie(movieInfo, newFilename);
    ...
    async _addMovieFileAssociations(movie, destInfo) {
        const destSetting = await this._settingsModel.getSingle("directories", "Movies");
        const possibleExisting = await this._fileToMovieModel.getSingleForMovie(movie.id);
        if ("file_id" in possibleExisting) {
            ...
            await this._fileToMovieModel.deleteSingle(possibleExisting.file_id, movie.id);
        }
        const fileRow = await this._filesModel.addFile(destSetting.id, destInfo.subpath, destInfo.filename, "movie");
    ...
    async idAndArchiveEpisode(epInfo, sourceInfo, destInfo) {
        ...
        const destSetting = await this._settingsModel.getSingle("directories", "Shows");
        // Remove possible existing files and associations
        await this._removeExistingEpisodeAssoc(epInfo.episode_id, destSetting.id, destInfo.subpath, destInfo.filename);
        // Add the new file
        const fileRow = await this._filesModel.addFile(destSetting.id, destInfo.subpath, destInfo.filename, "show");
        // Create the new association
        return await this._fileToEpisodeModel.add(fileRow.id, epInfo.show_id, epInfo.season_id, epInfo.episode_id);
    }
     ...
     * @param dest_filename
     */
    async _removeExistingEpisodeAssoc(episode_id, dest_setting_id, dest_subpath, dest_filename) {
        // Check if the episode is already associated
        const possibleLinks = await this._fileToEpisodeModel.getAllForEpisode(episode_id);
        ...
        for (const link of possibleLinks) {
            ...
            await this._fileToEpisodeModel.deleteSingleForEpisode(link.episode_id);
        }
        const possibleFiles = await this._filesModel.getAllForDirectoryAndFilename(dest_setting_id, dest_subpath, dest_filename);
    ...
    async _removeMovie(idInfo) {
        ...
        await this._fileToMovieModel.deleteSingle(idInfo.file_id, idInfo.movie_id);
    ...
    async _removeEpisodeFileAssociations(idInfo) {
        await this._filesModel.deleteSingle(idInfo.file_id);
        return await this._fileToEpisodeModel.deleteSingle(idInfo.file_id, idInfo.episode_id);
    }
    async addShow(showInfo, imageInfo) {
        const imageFilename = this._buildThumbFilename(showInfo);
        const newFilename = await this._mediaScraperModel.downloadThumbnail("shows", imageInfo.url, imageFilename);
    ...
    async _scrapeAndAddSeasonsForShow(show) {
        const seasons = await this._mediaScraperModel.getShowSeasonsList(show.trakt_id);
        const addedSeasons = await this._showSeasonsModel.addArrayOfSeasons(seasons, show.id);
    ...
    async _scrapeAndAddEpisodesForSeasons(show, seasons) {
        for (const season of seasons) {
            try {
                const episodesArr = await this._mediaScraperModel.getEpisodesForSeason(show.trakt_id, season.season_number);
                await this._showSeasonEpisodesModel.addArrEpisodes(show.id, season.id, episodesArr);
            }
            catch (err) {
                throw new Error("IDModel :: _scrapeAndAddEpisodesForSeasons + " + err);
            }
        }
    }
}
